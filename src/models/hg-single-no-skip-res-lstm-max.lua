...
local function hourglassNoSkip(n, numIn, numOut, inp)
    ...
    if n > 1 then
        ...
        -- Max-pooling
        local x1 = nnlib.SpatialMaxPooling(4,4,1,1)(low5)
        ...
        local h1 = nn.View(-1,hiddenSize,1,1)(hid)

        -- Up-sampling
        local h2 = nn.SpatialUpSamplingNearest(4)(h1)

        local add = nn.CAddTable()({low5,h2})

        low6 = Residual(256,numOut)(add)
    end
