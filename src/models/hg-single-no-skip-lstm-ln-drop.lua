...
local function hourglassNoSkip(n, numIn, numOut, inp)
    ...
    if n > 1 then
        ...
        -- Fully connected and view
        local x1_ = nn.View(-1,256*4*4)(low5)
        local x1 = nn.Linear(256*4*4,256)(x1_)
        local x1d = nn.Dropout(0.5)(x1)

        -- LSTM
        local x2 = nn.View(-1,1,256)(x1d)
        ...
        local h2d = nn.Dropout(0.5)(h2_)
        local h2 = nn.View(-1,256,4,4)(h2d)
